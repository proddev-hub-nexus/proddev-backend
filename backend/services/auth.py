...
from utils.auth import (VerifyEmailContext, 
                         hash_password, 
...
class AuthService:
    ...
    async def verify_email(self, token: str) -> dict:
        try:
            ...
            #create user dashboard after verification
            ...
            #save user and dashboard
            await user.save()
            await user_dashboard.insert()
            

            return {"message": "Email verification successful. You may now log in."}
    ...
    async def logout(self, user: User, response: Response) -> dict:
        """
        Logout the current user by clearing the access token cookie and removing the active token entry.
        """
        if not user:
            raise HTTPException(
                status_code=status.HTTP_401_UNAUTHORIZED,
        ...
        # Optional: clean active token list (if you're tracking per-device/session tokens)
        user.active_tokens = []  # or filter by device/token_id if needed
        await user.save()
